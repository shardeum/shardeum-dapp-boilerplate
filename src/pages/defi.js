...
function DeFi() {
    ...
    const [tokenAmount,setAmount]=useState("1")
    const isConnected=connected
    const tokenA = new ethers.Contract(tokenAAddress, erc20Abi, signer);
    const tokenAMint = new ethers.Contract(tokenAAddress, tokenAMintAbi, signer);
    ...
    async function mintAndSwapTokens() {
        try{
            const mintTx = await tokenAMint.mint(selectedAddress, amountIn);
            await mintTx.wait();
            ...
            notification.success({
                message:'Success',
                description:( <span>
      `Swap completed in transaction ${receipt.transactionHash}`
    </span>)
            })
    ...
    return (
        <div>
            ...
            <div className={styles.swapcard} style={{ width: 300 }}>
                <Text style={styleswap.textstyle}>Mint & Swap </Text>



                <Input className={styles.swapInput}type='number' placeholder='Amount of Token' name="tokenAmount" style={{width:200}} required onChange={(e)=>{setAmount(e.target.value)}}/>
                <Text style={styleswap.textstyle}></Text>


                <Button className={styles.swapButton} onClick={()=>{ if(isConnected){
                    mintAndSwapTokens()
                }
                else{
                    checkConnection(isConnected)
                }}} style={{width:200}} >Mint & Swap Token</Button>
            </div>
